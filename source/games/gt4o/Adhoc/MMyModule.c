...
{
    /* These adhoc constructors fill a handle slot, so they take the ADDRESS of
       the handle variable, not the handle itself. */
    HInt* hInt;
    HInt_HInt((HInt*)&hInt, __STDC_VERSION__);

    // Set return value
    ADHOC_MakeRef(return_value, (HObject*)hInt);

    // Clean up HInt
    HInt_dtor((HInt*)&hInt, 2);
}
